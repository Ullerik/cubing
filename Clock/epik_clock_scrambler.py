...
def scramble():
    ...
    if not selected_cases:
        messagebox.showwarning("No Case Selected", "Please select at least one case to test.")
        return

    while True:
        clock, scr = get_random_clock()
        if any(case(clock) for case in selected_cases):
            result_label.config(text=f"Scramble:\n{scr}\n{clock}", font=("Arial", font_size))
            break
...
scramble_button = tk.Button(app, text="Generate Scramble", command=scramble, font=("Arial", font_size))
scramble_button.pack()

result_label = tk.Label(app, text="", fg="blue", font=("Arial", font_size))
result_label.pack()

# Add explanation label
sign_label = tk.Label(app, text="Made by Ulrik Bredland (2012BRED01)", fg="black", font=("Arial", 8))
sign_label.pack()
